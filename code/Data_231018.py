...
def get_MNIST_loaders(data_dir, num_client, mali_client, logger, distribution, batch_size=32, p_stage=1.0, central_test=False):
    ...
    for i, (_, train_index) in enumerate(skf.split(indices, targets)):
        ...
        label_counts = pd.Series(train_labels + test_labels).value_counts()
        label_info = ', '.join([f"{label}: {count}" for label, count in label_counts.items()])
        logger.info(f"Client {i+1} Label Distribution: \n{label_info}")
    ...
    test_indx = list(range(len(mnist_test)))
...
def FashionMNIST_split_loader(logger, Fashionmnist_dataset, subset_indices, batch_size, p_stage=1.0, flip=False):
    # Filter out data only with label 1 and 7 for flipping
    if flip:
        # label 1 indices
        indices1_for_flip = [subset_indices[i] for i in range(len(subset_indices)) if Fashionmnist_dataset.targets[subset_indices[i]] == 1]
        samples1_flip = int(len(indices1_for_flip) * p_stage)

        # label 7 indices
        indices7_for_flip = [subset_indices[i] for i in range(len(subset_indices)) if Fashionmnist_dataset.targets[subset_indices[i]] == 7]
        samples7_flip = int(len(indices7_for_flip) * p_stage)
        ...
        # Flip the labels
        for idx in indices1_for_flip:
            Fashionmnist_dataset.targets[idx] = 7
        for idx in indices7_for_flip:
            Fashionmnist_dataset.targets[idx] = 1
        
        # log
        logger.info(f'Flipped {samples1_flip} label 1 samples and {samples7_flip} label 7 samples.')

        # for idx in tqdm(indices_for_flip, desc='Flipping'):
        #     if Fashionmnist_dataset.targets[idx] == 7:
        #         Fashionmnist_dataset.targets[idx] = 1 
        #     elif Fashionmnist_dataset.targets[idx] == 1:
        #         Fashionmnist_dataset.targets[idx] = 7
    ...
    # Create Datasets
    train_dataset = Subset(Fashionmnist_dataset, train_indices)
    test_dataset = Subset(Fashionmnist_dataset, test_indices)
...
def get_FashionMNIST_loaders(data_dir, num_client, mali_client, logger, distribution, batch_size=32, p_stage=1.0, central_test=False):
    # Load FashionMNIST dataset
    transform = transforms.Compose([transforms.ToTensor(),
                                transforms.Normalize([0.5], [0.5])
                               ])
    Fashionmnist_dataset = datasets.FashionMNIST(data_dir, train=True, download=True, transform=transform)
    ...
    targets = [label for _, label in Fashionmnist_dataset]
    ...
    for i, (_, train_index) in enumerate(skf.split(indices, targets)):
        if len(loaders_train) < num_client - mali_client:
            train_loader, test_loader, train_dataset, test_dataset  = FashionMNIST_split_loader(logger, Fashionmnist_dataset, train_index, batch_size, p_stage)
            loaders_train.append(train_loader)
            loaders_test.append(test_loader)
            malicious_info.append(False)
            central_trainset.append(train_dataset)
            ...
            logger.info(f'Client {len(loaders_train)+1} is malicious.')
            train_loader, test_loader, train_dataset, test_dataset  = FashionMNIST_split_loader(logger, Fashionmnist_dataset, train_index, batch_size, p_stage, flip=True)
            loaders_train.append(train_loader)
            loaders_test.append(test_loader)
            malicious_info.append(True)
            central_trainset.append(train_dataset)
    ...
    # Test set loader
    Fashionmnist_test = datasets.FashionMNIST(data_dir, train=False, download=True, transform=transforms.ToTensor())
    test_indx = list(range(len(Fashionmnist_test)))
    test_loader = DataLoader(Fashionmnist_test, num_workers=2, batch_size=batch_size, shuffle=False)
