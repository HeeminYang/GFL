...
def agg_test(log, start_point, end_point, agg_dict, info_dict):
    ...
    for i in range(start_point, end_point):
        ...
        if switch: 
            ...
            if ("Brier Score for each label:" in log[i+1]):
                agg_dict['Brier0'].append(float(log[i+1].split(" ")[9].replace(",", "")))
                agg_dict['Brier1'].append(float(log[i+1].split(" ")[11].replace(",", "")))
                agg_dict['Brier2'].append(float(log[i+1].split(" ")[13].replace(",", "")))
                agg_dict['Brier3'].append(float(log[i+1].split(" ")[15].replace(",", "")))
                agg_dict['Brier4'].append(float(log[i+1].split(" ")[17].replace(",", "")))
                agg_dict['Brier5'].append(float(log[i+1].split(" ")[19].replace(",", "")))
                agg_dict['Brier6'].append(float(log[i+1].split(" ")[21].replace(",", "")))
                agg_dict['Brier7'].append(float(log[i+1].split(" ")[23].replace(",", "")))
                agg_dict['Brier8'].append(float(log[i+1].split(" ")[25].replace(",", "")))
                agg_dict['Brier9'].append(float(log[i+1].split(" ")[27].replace("}\n", "")))
            if ("Brier Score:" in log[i+1]):
                agg_dict['Brier'].append(float(log[i+1].split(" ")[4]))
...
def save_csv(agg_df, file_name):
    ...
    agg_df[['Accuracy', 'Brier', 'Recall', 'malicious']].to_csv(f"{file_name}_R.csv", index=False)
    agg_df[['Brier0', 'Recall0', 'malicious']].to_csv(f"{file_name}_R0.csv", index=False)
    agg_df[['Brier6', 'Recall6', 'malicious']].to_csv(f"{file_name}_R6.csv", index=False)
...
# main
def main():
    path = "/home/heemin/GFL/log_nonFL"
    file_name = 'data_B'
    ...
    dctree(agg_df[['Accuracy', 'Brier', 'Recall', 'malicious']], file_name+'_R', 42)
    dctree(agg_df[['Brier0', 'Recall0', 'Brier6', 'Recall6', 'malicious']], file_name+'_R0R6', 42)
    dctree(agg_df[['Brier0', 'Recall0', 'malicious']], file_name+'_R0', 42)
    dctree(agg_df[['Brier6', 'Recall6', 'malicious']], file_name+'_R6', 42)
