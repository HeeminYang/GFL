...
if __name__ == '__main__':
    ...
    if rank == 0:
        ...
        for i in range(args.client):
            clients.append(copy.deepcopy(client(args, gan_list[i][0], gan_list[i][1], train_loaders[i], test_loaders[i], 
                                                DEVICE, logger, args.source, args.target, copy.deepcopy(CNN()), copy.deepcopy(CNN()), i))) # , external_loaders[0]
        # run
        # save_path = f'./project'
    ...
    for Round in range(args.round):
        ...
        if (Round + 1) % args.warm_up == 0:
            ...
            if rank == 0:
                ...
                my_client.local_test(generated_loaders, f'{save_path}/{args.project_name}/wrong/client{rank}', Round, W_k, rank)
            comm.Barrier()
    ...
    if rank !=0:
        my_client.make_plot(f'{save_path}/{args.project_name}/result/client{rank}')
